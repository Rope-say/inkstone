...
> **用途**：对接 Cloudflare Workers AI 实现 RAG 知识库
> **适用**：Obsidian + Inkstone 异地云端备用方案
> **注意**：所有密钥建议设置为【保密环境变量 Secret】
...
1. **AccountID（账户ID）**：位置在 Cloudflare 控制面板右上角头像菜单中复制；拼接在URL路径中，用于定位账户资源。
2. **账户API令牌（Bearer Token）**：需自行创建，授予「账户级 Workers AI 编辑权限」，作为接口调用鉴权凭证；URL提供访问地址，令牌提供访问权限，二者必须归属同一个Cloudflare账户。
...
1. 修改 `LLM_MODEL`：可自由切换模型，**无需重建知识库向量索引**；
2. 修改 `EMBEDDING_MODEL`：因向量维度变更，**必须清空知识库，重新全部向量化**。
...
**在 `https://github.com/Rope-say/inkstone` 仓库中完成：**
- 通过网页 `Add file → Create new file`，新建 `.github/workflows/deploy.yml` 文件
- 写入工作流配置，用于在 `main` 分支推送时自动触发部署

### 2. 配置 Cloudflare API 令牌

- 在 Cloudflare Dashboard → 个人资料 → API 令牌 中创建新令牌
- 授予以下 **Write** 权限：
  - Workers Scripts
  - Workers KV Storage
  - Workers R2 Storage
...
- 添加 `CLOUDFLARE_API_TOKEN` Secret，值为上一步创建的 API 令牌

### 4. 验证

- 每次 `git push` 到 `main` 分支，GitHub Actions 将自动执行部署
- `wrangler.toml` 中的 `[ai]` 和 `[[vectorize]]` 绑定将通过 CI 流程生效
